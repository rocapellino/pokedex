...
| **Manipulación DOM (XSS)** | [`apps/frontend/src/*.ts`](../../apps/frontend/src/pokedex.ts) | **Alta** | Renderizado de respuestas de API en el DOM vía `innerHTML`. | **Mitigado:** Función `escapeHTML` estricta sanitizando `<`, `>`, `&`, `"`, `'` y backticks. |
...
| **Rotación de Credenciales** | Rotación periódica (60/90 días) y de emergencia con ExternalSecret (Reloader en Cloud / `checksum/config` en Proxmox, formalizado en [secret-rotation.md](../operations/secret-rotation.md)) | Implementado | Reducción de la ventana de exposición ante fugas y recargas automáticas sin downtime. |
