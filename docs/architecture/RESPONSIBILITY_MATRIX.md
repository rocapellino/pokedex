...
| **Bastion Host (LXC 802)** | Punto único de entrada administrativa (Management Plane), sesión interactiva restringida, auditoría inmutable de comandos y ejecución de *Break-Glass*. | On-prem (10.10.13.111) | **Sí** | Si se elimina, los operadores accederían directamente a los nodos de cómputo y clúster K8s sin registro de auditoría (`/var/log/bastion/audit.log`), violando Zero-Trust. |
| **HashiCorp Vault CE (LXC 110)** | Secret Manager on-premise en caliente con almacenamiento transaccional Raft, esquema Shamir 5/3, TLS y particionamiento lógico multi-ambiente. | On-prem (10.10.13.110) | **Sí** | Si se elimina, los secretos deberían guardarse en texto plano en disco o en Git, violando las políticas de Zero-Disk y el principio de no-persistencia de credenciales. |
