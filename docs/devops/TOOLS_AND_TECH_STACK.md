...
| **Automatización DX** | **Taskfile (go-task)** | Comandos unificados 100% cross-platform (`task test`, `task dev`, `task audit`, `task ts:dev`, etc.) | [`Taskfile.yml`](file:///Taskfile.yml) |
...
* **Taskfile (`task` - go-task):**
  * *¿Para qué sirve?* Unificar comandos largos entre Windows, Linux y macOS (ej: `task dev`, `task test`, `task lint`, `task docker:up`, `task ts:dev`) mediante un único ejecutable sin dependencias de shells UNIX ni emuladores.
* **Linear MCP Server:**
